...
HWTree::HWTree()
{
    ...
    for(int i=0;i<=jmax;i++)
        tree[i].resize(2*i+1);
    for(int i=jmax+1;i>jmax&&i<N+1;i++)
        tree[i].resize(2*jmax+1);
    
    for(int i=0;i<=jmax;i++)
        for(int j=0;j<2*i+1;j++)
            tree[i][j] = (i-j) * delta_x;
    for(int i=0;i>jmax&&i<N+1;i++)
        for(int j=jmax+1;j<2*jmax+1;j++)
...
void HWTree::get_gamma()
{
    ...
    for(int step=jmax+1;step<N;step++)
    {
        ...
        r_tudi[step][0] = r_tudi[step-1][0] * top_prob(0)[0] * exp(-(gamma[step-1] + 3 * delta_x) * delta_t);
        r_tudi[step][0] += r_tudi[step-1][1] * normal_prob(jmax,1)[0] * exp(-(gamma[step-1] + 2 * delta_x) * delta_t);
        
        r_tudi[step][1] = r_tudi[step-1][0] * top_prob(0)[1] * exp(-(gamma[step-1] + 3 * delta_x) * delta_t);
        r_tudi[step][1] += r_tudi[step-1][1] * normal_prob(jmax,1)[1] * exp(-(gamma[step-1] + 2 * delta_x) * delta_t);
        r_tudi[step][1] += r_tudi[step-1][2] * normal_prob(jmax,2)[0] * exp(-(gamma[step-1] + 1 * delta_x) * delta_t);
        
        
        r_tudi[step][2] = r_tudi[step-1][0] * top_prob(0)[2] * exp(-(gamma[step-1] + 3 * delta_x) * delta_t);
        for(int i=1;i<4;i++)
            r_tudi[step][2] += r_tudi[step-1][i] * normal_prob(jmax,i)[3-i] * exp(-(gamma[step-1] + (3-i) * delta_x) * delta_t);
        
        r_tudi[step][3] = 0;
        for(int i=0;i<3;i++)
            r_tudi[step][3] += r_tudi[step-1][2+i] * normal_prob(jmax,2+i)[2-i] * exp(-(gamma[step-1] + (1-i) * delta_x) * delta_t);
        
        r_tudi[step][4] = r_tudi[step-1][6] * bot_prob(6)[0] + exp(-(gamma[step-1] - 3 * delta_x) * delta_t);
        for(int i=0;i<3;i++)
            r_tudi[step][4] += r_tudi[step-1][3+i] * normal_prob(jmax,3+i)[2-i] * exp(-(gamma[step-1] - i * delta_x) * delta_t);
        
        r_tudi[step][5] = r_tudi[step-1][4] * normal_prob(jmax,4)[2] * exp(-(gamma[step-1] - delta_x) * delta_t);
        r_tudi[step][5] += r_tudi[step-1][5] * normal_prob(jmax,5)[1] * exp(-(gamma[step-1] - 2 * delta_x) * delta_t);
        r_tudi[step][5] += r_tudi[step-1][6] * bot_prob(6)[1] * exp(-(gamma[step-1] - 3 * delta_x) * delta_t);
        
        r_tudi[step][6] = r_tudi[step-1][5] * normal_prob(jmax,5)[2] * exp(-(gamma[step-1] - 2 * delta_x) * delta_t);
        r_tudi[step][6] += r_tudi[step-1][6] * bot_prob(2*jmax)[2] * exp(-(gamma[step-1] - 3 * delta_x) * delta_t);
        
        double temp = 0;
        /*for(int i=0;i<2*jmax+1;i++)
            temp += r_tudi[step][i] * exp(-(jmax-i) * delta_x *delta_t); */
        
        temp = r_tudi[step][0] * exp(-3 * delta_x * delta_x) + r_tudi[step][1] * exp(-2 * delta_x * delta_x) + r_tudi[step][2] * exp(-1 * delta_x * delta_x) + r_tudi[step][3] + r_tudi[step][4] * exp(1 * delta_x * delta_x) + r_tudi[step][5] * exp(2 * delta_x * delta_x) + r_tudi[step][6] * exp(3 * delta_x * delta_x);
        
        gamma[step] = (log(temp)/delta_t + bond_p[step+1] * (step+1)) / 2000;
